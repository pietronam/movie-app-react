...
describe("mapContent", () => {
  ...
  it("should return an empty array upon receiving one", () => {
    expect(Array.isArray(mapContent([]))).toBe(true);
  });

  it("should throw an error when not receiving any arguments", () => {
    expect(() => mapContent()).toThrow();
  });

  it("should throw an error when receiving undefined", () => {
    expect(() => mapContent(undefined)).toThrow();
  });
});
